...
gestionePortafoglio = function(#input
  ...
  calcoloDecessi = function()
  {
    ...
    for (i in eta:(anniCopertura + eta))
    {
      ...
      mortiCasuali = sum(rpois(numeroAssicurati, mu))
      # mortiCasuali = numeroAssicurati * mu  # se volessimo fare un modello deterministico
...
server <- function(input, output) {
  ...
  callPortafoglio = reactive(
  {
    return(gestionePortafoglio(eta = input$eta,
                               ...
                               tavolaPeriodo = lifecontingencies::demoIta[,names(demoIta) == input$tavolaPeriodo], 
                               tavolaMortalita = lifecontingencies::demoIta[,names(demoIta) == input$tavolaTecnica]))
    ...
    output$monteCarloPlot = renderPlot({
      ...
      for(i in 1:1E3)
      {
        ...
        v = gestionePortafoglio(eta = input$eta,
                            ...
                            rendimentoFondoAnnuo = input$rendimentoFondoAnnuo,
                            tavolaPeriodo = lifecontingencies::demoIta[,names(demoIta) == input$tavolaPeriodo], 
                            tavolaMortalita = lifecontingencies::demoIta[,names(demoIta) == input$tavolaTecnica])$andamentoFondo
        x = c(x,v[length(v)])
      }
      x %>% 
        tibble %>% 
